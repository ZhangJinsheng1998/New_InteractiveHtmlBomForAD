...
12. **元器件盒面板（`web/ibom.html`、`web/ibom.css`、`web/ibom.js`）**
    - 左侧 BOM 区分成上下两部分：上面 BOM 表（独立滚动），下面"元器件盒"网格，
      对照实物元器件收纳盒使用。默认 8 行 × 16 列，行/列数可在面板顶部修改（1~50），可折叠。
    - 网格带行号、列号表头（滚动时表头吸附）。
    - 存放：点击 BOM 行锁定后，单击某格把该行元器件放入（格子显示首个位号，多个显示 `R32+3`，
      悬停提示完整位号列表）；再次单击已存放它的格子取出；一个位号只能存在一个格子里。
    - 联动：点击 BOM 行时，对应格子橙色高亮并滚动到可见位置；反过来，无锁定行时单击已存放的
      格子会反查锁定对应 BOM 行、缩放定位 PCB。
    - 编辑：双击格子弹出输入框手动填写位号（逗号分隔，留空清除）；右键格子直接清空。
    - 数据按"板卡标题+版本"存到浏览器 localStorage，刷新/重开页面不丢失。
