...
生成的网页在原版基础上增加了「点击定位」和「元器件盒」两大功能，详细的实现记录见下方更新日志。

**BOM 与 PCB 联动**

| 操作 | 效果 |
|---|---|
| 悬停 BOM 行 | 临时高亮 PCB 上对应元件（原版行为） |
| **点击 BOM 行** | 锁定该行（深绿+边框），PCB 自动缩放居中到元件；悬停不再抢走高亮 |
| 再次点击锁定行 / 点击 PCB 或 BOM 空白处 | 解锁 |
| 点击 PCB 上的元件 | 跳转并锁定对应 BOM 行 |
| 键盘 ↑/↓ | 上下切换行（锁定跟随） |
| 设置菜单「点击定位最大放大」 | 调整点击定位的缩放上限（默认 4 倍，1~20 可调） |

**元器件盒**（左下面板，数据全局共用、所有板卡的网页互通）

| 操作 | 效果 |
|---|---|
| 锁定 BOM 行后点击**空格子** | 把该行元器件（值+封装）存入格子 |
| 点击**有料的格子** | 跳转联动：锁定对应 BOM 行 + PCB 缩放定位 |
| 点击存放着当前锁定元器件的格子 | 取出（清空该格） |
| **双击格子** | 居中对话框编辑：连写（`1K0603`、`226100805`）自动拆值/耐压/封装，实时预览「识别为」；类型、封装可自动/手选 |
| 右键格子 | 清空该格 |
| 顶部下拉框 / ＋ / 改名 / 删 | 多盒子管理；点击 BOM 时自动切到元器件所在的盒子 |
| 导出 / 导入 | 备份、换电脑、分享盒子数据（`parts-bin.json`） |

标注含义：BOM 行**淡蓝色背景** = 盒子里有这个料；PCB 上**蓝色焊盘** = 同上；格子**红字** = 与其它格子重复；
格子**绿色描边** = 当前锁定行对应的存放位置。匹配是模糊的：`4K7`=`4.7K`、`104`=`100nF`、`5mR`=`0.005R`、
`10uF` 通配任何耐压、`RT9080` 匹配 `RT9080-33GB`，详见更新日志第 13 条。
