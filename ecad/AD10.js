...
function parsePcb(config) {
    ...
    function parsePad(Prim) {
        ...
        if (Prim.Layer == eMultiLayer) {
            switch (Prim.TopShape) {
                ...
                case 9:  // Rounded Rectangle in AD
                    ...
                    break;
                default:
                    res["shape"] = "rect";  // unknown/special pad shape -> fall back to rect to avoid missing polygons
            }
            ...
            switch (Prim.BotShape) {
                ...
                case 9:
                    ...
                    break;
                default:
                    res["shape"] = "rect";  // unknown/special pad shape -> fall back to rect
            }
        } 
        else {
        ...
        if (res["shape"] == "chamfrect") {  // AD octagonal pad: chamfer all 4 corners to approximate a regular octagon
            res["radius"] = 0;          // all corners chamfered, radius unused
            res["chamfpos"] = 15;       // bitmask: left1 right2 bottom-left4 bottom-right8, all corners
            res["chamfratio"] = 0.2929; // 1/(2+sqrt(2)), makes the 8 edges equal length
