...
function drawText(ctx, text, color) {
  ...
  // 文本对象缺少文字内容（如隐藏文本/条码文本被导出成空对象）时直接跳过，避免崩溃。
  if (!("svgpath" in text) && (text.text === undefined || text.text === null)) return;
  ctx.save();
  ctx.fillStyle = color;
  ...
  for (var i in txt) {
    ...
    for (var j = 0; j < txt[i].length; j++) {
      if (txt[i][j] == '\t') {
        ...
        var lineGlyph = pcbdata.font_data[txt[i][j]];
        if (lineGlyph) lineWidth += lineGlyph.w * text.width;
      }
    }
    ...
    for (var j = 0; j < txt[i].length; j++) {
      ...
      // 字库中没有该字符时跳过，避免读取 undefined.w / undefined.l 报错。
      if (!glyph) continue;
      if (inOverbar) {
...
function getPolygonsPath(shape) {
  ...
  if ("svgpath" in shape) {
    ...
    if (shape.polygons) {
      ...
      // 异形/未知焊盘没有 polygons 数据时，按 size 退化成矩形，
      // 避免 "shape.polygons is not iterable" 中断整块板的渲染。
      path.rect(...shape.size.map(c => -c * 0.5), ...shape.size);
    }
    shape.path2d = path;
  }
  return shape.path2d;
...
function drawBgLayer(layername, canvas, layer, scalefactor, edgeColor, polygonColor, textColor) {
  ...
  for (var d of pcbdata[layername][layer]) {
    try {
      ...
      // 单个图元绘制失败时跳过，避免中断整层渲染。
      console.warn("跳过无法绘制的 " + layername + " 图元：", e, d);
    }
  }
}
